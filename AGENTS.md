...
- **Sites (Layer 2):** clients have **view-only** access (`user_can_manage_site` excludes clients). They reach **`/dashboard/sites`** via tab **Sites (view)** or dashboard **View sites**; ERF machine icons open **`MachineDetailView`** without layout edit controls. **Charts:** Sites must use the same `useMachineData` historical prefetch + `historicalData[machineId]` as **Machines** dashboard (see `AccountsContext.md` §4.1); detail overlay uses portal + body scroll lock.
