...
const generateHistoricalData = (currentValue: number, variance: number, hours: number = 24) => {
  ...
  const intervalMs = (hours * 60 * 60 * 1000) / points; // Spread across the time period
      ...
      try {
        ...
        if (userRole === 'super_admin' || userRole === 'company' || userRole === 'installer') {
          ...
          visibleMachines = (allMachines || []).map((m: any) => ({
            ...
            voltage: m.voltage,
            power: m.power,
            temperatureSetpoint: m.temperature_setpoint,
        ...
        // Generate historical data for each machine (24 hours by default)
        const historical: Record<string, MachineHistoricalData> = {};
        visibleMachines.forEach(machine => {
          // Generate random on/off states throughout the day
          const fanStates = Array(24 * 60).fill(0).map(() => Math.random() > 0.5 ? 30 : 0);
          const coolingStates = Array(24 * 60).fill(0).map(() => Math.random() > 0.6 ? 30 : 0);
          const waterStates = Array(24 * 60).fill(0).map(() => Math.random() > 0.8 ? 0 : 20);
          
          historical[machine.id] = {
            power: generateHistoricalData(machine.power, 100, 24),
            deltaT: generateHistoricalData(machine.deltaT, 2, 24),
            motorTemp: generateHistoricalData(machine.motorTemp, 5, 24),
            current: generateHistoricalData(machine.current, 2, 24),
            outsideTemp: generateHistoricalData(machine.outsideTemp, 3, 24),
            insideTemp: generateHistoricalData(machine.insideTemp, 3, 24),
            fanActive: fanStates.map((value, i) => ({
              timestamp: Date.now() - (24 * 60 - i) * 60 * 1000,
              value
            })),
            isCooling: coolingStates.map((value, i) => ({
              timestamp: Date.now() - (24 * 60 - i) * 60 * 1000,
              value
            })),
            hasWater: waterStates.map((value, i) => ({
              timestamp: Date.now() - (24 * 60 - i) * 60 * 1000,
              value
            }))
          };
