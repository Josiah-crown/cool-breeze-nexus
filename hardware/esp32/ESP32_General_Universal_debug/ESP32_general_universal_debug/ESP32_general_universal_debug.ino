/*
 * ============================================
 * ESP32 - UNIVERSAL (All Manufacturers) - DEBUG BUILD
 * Cool Breeze Nexus Integration
 * ============================================
 *
 * VERSION: 4.0.0-UNIVERSAL-DEBUG
 * COMPATIBILITY: ESP32 Arduino Core 2.0.11 - 3.0.x
 *
 * Same as ESP32_General_Universal.ino but with full serial debugging.
 * Debug mode can be toggled via WiFiManager config page: "Debug (1=On 0=Off)".
 * Setting is saved in Preferences and persists across reboots.
 *
 * MANUFACTURERS SUPPORTED: Cirrus, CoolBreeze, Alliance, future.
 * GPIO: 6 voltage inputs (see main Universal sketch).
 * NOTE: No 6-month full reset (would wipe machine_id/API key).
 *
 * ============================================
 */
...
// ============================================
// DEBUG - Runtime toggle (saved in Preferences, set via WiFiManager)
// ============================================
bool debugMode = true;  // Default ON for debug build; load from prefs in setup

#define DLOG(msg)   do { if(debugMode) Serial.print(msg); } while(0)
#define DLOGLN(msg) do { if(debugMode) Serial.println(msg); } while(0)

// ============================================
// DEVICE CONFIGURATION
// ============================================
const String DEVICE_ID = "UNIVERSAL";
const String FIRMWARE_VERSION = "4.0.0-UNIVERSAL-DEBUG";
...
const unsigned long DEBUG_STATUS_INTERVAL_MS = 3000;   // One debug status block every 3s
...
const int GPIO_INPUT_1 = 35;  // Green - Fan
const int GPIO_INPUT_2 = 32;  // Brown - Pump
const int GPIO_INPUT_3 = 33;  // Black - Drain
const int GPIO_INPUT_4 = 34;  // Yellow - Exhaust
const int GPIO_INPUT_5 = 5;   // FLOAT or Heat Relay
const int GPIO_INPUT_6 = 25;   // Optional
...
const char* PREF_DEBUG_MODE = "debug_mode";
...
unsigned long wifiConnectStartTime = 0;  // set when WiFi connect starts, 0 when not connecting
...
bool motorTempOnline = false;
bool exteriorTempOnline = false;
bool interiorTempOnline = false;
...
void setup() {
  ...
  Serial.println("\n\n============================================");
  Serial.println("ESP32 UNIVERSAL DEBUG - All Manufacturers");
  ...
  DLOGLN("[DEBUG] Config loaded: machineId=" + machineId + " debugMode=" + String(debugMode ? 1 : 0));

  initializeSensors();
  DLOGLN("[DEBUG] Sensors initialized");

  initializeWiFiManager();
  DLOGLN("[DEBUG] WiFiManager init done");
  ...
  DLOGLN("[DEBUG] Debug mode is " + String(debugMode ? "ON" : "OFF") + " (change via WiFiManager config page)");
}
...
void loop() {
  ...
  if(currentMillis - lastAutoReset >= AUTO_RESET_INTERVAL) {
    ...
    DLOGLN("[DEBUG] lastAutoReset=" + String(lastAutoReset) + " interval=" + String(AUTO_RESET_INTERVAL));
    delay(1000);
    ESP.restart();
  }
  ...
  // Debug: one status block every 3s. Read-only - does not change state, timing, or WiFi/sensors.
  {
    static unsigned long lastDebugStatus = 0;
    if (debugMode && currentMillis - lastDebugStatus >= DEBUG_STATUS_INTERVAL_MS) {
      lastDebugStatus = currentMillis;
      Serial.println("---");
      Serial.print("state=");
      switch (currentState) {
        case STATE_INITIAL_WIFI_CONNECT: Serial.print("INIT_WIFI"); break;
        case STATE_SENSOR_READING: Serial.print("SENSOR"); break;
        case STATE_WIFI_CONNECT:   Serial.print("WIFI_CONN"); break;
        case STATE_DATA_SEND:      Serial.print("DATA_SEND"); break;
        case STATE_WIFI_DISCONNECT: Serial.print("WIFI_DISC"); break;
      }
      Serial.print(" wifi=");
      ...
      } else if (currentState == STATE_WIFI_CONNECT && wifiConnectStartTime != 0) {
        Serial.print("connecting ");
        Serial.print((WIFI_CONNECT_TIMEOUT - (currentMillis - wifiConnectStartTime)) / 1000);
        Serial.print("s left");
      } else {
        Serial.print("off");
      }
      Serial.println();
      Serial.print("T_mot="); Serial.print(motorTemp, 1); Serial.println(motorTempOnline ? " ok" : " OFFLINE");
      Serial.print("T_ext="); Serial.print(exteriorTemp, 1); Serial.println(exteriorTempOnline ? " ok" : " OFFLINE");
      Serial.print("T_int="); Serial.print(interiorTemp, 1); Serial.println(interiorTempOnline ? " ok" : " OFFLINE");
      Serial.print("CT="); Serial.print(current, 2); Serial.println(" A");
      Serial.print("V1="); Serial.print(voltage1, 1); Serial.print(" V2="); Serial.print(voltage2, 1);
      Serial.print(" V3="); Serial.print(voltage3, 1); Serial.print(" V4="); Serial.print(voltage4, 1);
      Serial.print(" V5="); Serial.print(voltage5, 1); Serial.print(" V6="); Serial.println(voltage6, 1);
      Serial.print("n="); Serial.println(sensorReadCount);
      Serial.println("---");
    }
  }
...
void stateInitialWiFiConnect(unsigned long currentMillis) {
  ...
  if(!attemptStarted) {
    ...
    DLOGLN("[DEBUG] STATE_INITIAL_WIFI_CONNECT: starting attempt");
    WiFi.mode(WIFI_STA);
    WiFi.begin();
    attemptStarted = true;
  ...
  if(WiFi.status() == WL_CONNECTED) {
    ...
    DLOGLN("[DEBUG] Initial WiFi OK - disconnecting, entering SENSOR_READING");
    WiFi.disconnect();
    WiFi.mode(WIFI_OFF);
    delay(200);
  ...
  if(currentMillis - attemptStart >= WIFI_INITIAL_CONNECT_TIMEOUT_MS) {
    if(currentMillis - phaseStartTime >= INITIAL_CONNECT_GIVE_UP_MS) {
      ...
      DLOGLN("[DEBUG] Initial phase 5 min give-up - clearing credentials, restart");
    ...
    DLOGLN("[DEBUG] Initial connection timeout - retrying");
    WiFi.disconnect();
    delay(500);
    WiFi.begin();
    attemptStart = currentMillis;
...
void stateWiFiConnect(unsigned long currentMillis) {
  ...
  if(WiFi.status() == WL_CONNECTED) {
    ...
    DLOGLN("[DEBUG] WiFi connected IP=" + WiFi.localIP().toString() + " RSSI=" + String(WiFi.RSSI()));
    wifiEnabled = true;
    wifiOnStartTime = millis();
    connecting = false;
    wifiConnectStartTime = 0;
    currentState = STATE_DATA_SEND;
  } else if(wifiConnectStartTime != 0 && (millis() - wifiConnectStartTime >= WIFI_CONNECT_TIMEOUT)) {
    Serial.println("✗ WiFi connection failed");
    DLOGLN("[DEBUG] WiFi timeout after " + String(WIFI_CONNECT_TIMEOUT) + " ms");
    connecting = false;
    wifiConnectStartTime = 0;
    currentState = STATE_SENSOR_READING;
  }
...
void stateDataSend(unsigned long currentMillis) {
  ...
  if(!sending && wifiEnabled && WiFi.status() == WL_CONNECTED) {
    ...
    DLOGLN("[DEBUG] Sending: avg T_mot=" + String(avgMotorTemp, 2) + " T_ext=" + String(avgExteriorTemp, 2) + " T_int=" + String(avgInteriorTemp, 2) + " I=" + String(avgCurrent, 2));
    ...
    if(success) {
      ...
      DLOGLN("[DEBUG] sendDataToSupabase returned false");
    }
...
void readSensors() {
  ...
  float rawMot = sensorMotor.getTempCByIndex(0);
  float rawExt = sensorExterior.getTempCByIndex(0);
  float rawInt = sensorInterior.getTempCByIndex(0);
  motorTempOnline = isValidTemperature(rawMot);
  exteriorTempOnline = isValidTemperature(rawExt);
  interiorTempOnline = isValidTemperature(rawInt);
  ...
  voltage1 = readVoltage(GPIO_INPUT_1);
  voltage2 = readVoltage(GPIO_INPUT_2);
  voltage3 = readVoltage(GPIO_INPUT_3);
  voltage4 = readVoltage(GPIO_INPUT_4);
  voltage5 = readVoltage(GPIO_INPUT_5);
  voltage6 = readVoltage(GPIO_INPUT_6);
}
...
bool sendDataToSupabase(float motorTemp, float outsideTemp, float insideTemp, float current) {
  if(WiFi.status() != WL_CONNECTED) {
    DLOGLN("[DEBUG] sendDataToSupabase: WiFi not connected");
    return false;
  }
  ...
  doc["current"] = current;
  doc["voltage_input_1"] = voltage1;
  doc["voltage_input_2"] = voltage2;
  doc["voltage_input_3"] = voltage3;
  doc["voltage_input_4"] = voltage4;
  doc["voltage_input_5"] = voltage5;
  doc["voltage_input_6"] = voltage6;
  ...
  DLOGLN("[DEBUG] POST " + url);
  DLOGLN("[DEBUG] Payload: " + jsonBody);

  esp_task_wdt_reset();
  int httpResponseCode = http.POST(jsonBody);

  DLOGLN("[DEBUG] HTTP response: " + String(httpResponseCode));
...
void checkBootButton(unsigned long currentMillis) {
  ...
  if(buttonState && !bootButtonPressed) {
    ...
    DLOGLN("[DEBUG] Boot button pressed");
    ...
    if(currentMillis - bootButtonPressTime >= BOOT_BUTTON_HOLD_TIME) {
      ...
      DLOGLN("[DEBUG] Boot button held 10s - clearing WiFi credentials");
      preferences.remove(PREF_WIFI_SSID);
      preferences.remove(PREF_WIFI_PASS);
      WiFi.disconnect(true);
      wifiManager.resetSettings();
    ...
    DLOGLN("[DEBUG] Boot button released");
  }
}
...
void checkWiFiStuck(unsigned long currentMillis) {
  if(wifiEnabled && WiFi.status() != WL_CONNECTED) {
    if(!wifiStuckDetected) {
      ...
      DLOGLN("[DEBUG] WiFi stuck detected - starting " + String(WIFI_STUCK_TIMEOUT) + " ms timer");
    } else if(currentMillis - wifiStuckStartTime > WIFI_STUCK_TIMEOUT) {
      Serial.println("WiFi stuck - forcing reset!");
      DLOGLN("[DEBUG] WiFi stuck timeout - ESP.restart()");
      delay(1000);
      ESP.restart();
    }
  } else {
...
void initializeWiFiManager() {
  ...
  String customMenu = "<div style='text-align: center; padding: 20px;'>"
    ...
    "<p style='font-size: 12px; color: #666;'>Debug: set 1=On 0=Off below and Save</p>"
    "</div>";
  ...
  char debugBuf[4];
  snprintf(debugBuf, sizeof(debugBuf), "%d", debugMode ? 1 : 0);
  ...
  WiFiManagerParameter custom_debug("debug_mode", "Debug (1=On 0=Off)", debugBuf, 4);
  ...
  if(savedSSID.length() == 0) {
    ...
    // Have credentials: do not connect here; initial connection in loop (STATE_INITIAL_WIFI_CONNECT, 15s attempt, retry until success)
    (void)savedSSID;
    (void)savedPass;
  }
  ...
  String debugStr = String(custom_debug.getValue());
  debugStr.trim();
  if(debugStr.length() > 0) {
    bool newDebug = (debugStr == "1" || debugStr.equalsIgnoreCase("on") || debugStr.equalsIgnoreCase("true"));
    if(newDebug != debugMode) {
      debugMode = newDebug;
      DLOGLN("[DEBUG] Debug mode set from config page: " + String(debugMode ? "ON" : "OFF"));
    }
  }
  ...
  DLOGLN("[DEBUG] Debug mode saved: " + String(debugMode ? 1 : 0));
}
...
void loadConfiguration() {
  ...
  debugMode = preferences.getBool(PREF_DEBUG_MODE, true);  // default ON for debug build
}

void saveConfiguration() {
  preferences.putString(PREF_MACHINE_ID, machineId);
  preferences.putString(PREF_API_KEY, apiKey);
  preferences.putBool(PREF_DEBUG_MODE, debugMode);
}
...
void updateUptime() {
  ...
  if (elapsed >= 1000) {
    ...
    if(debugMode && currentMillis - lastUptimeLog >= 60000) {
      lastUptimeLog = currentMillis;
      DLOGLN("[DEBUG] Uptime total (s): " + String(totalUptime));
    }
  }
}
