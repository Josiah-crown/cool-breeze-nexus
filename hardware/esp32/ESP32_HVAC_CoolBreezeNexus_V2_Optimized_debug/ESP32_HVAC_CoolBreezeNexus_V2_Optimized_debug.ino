...
void stateDataSend(unsigned long currentMillis) {
  ...
  if(supabaseUrl.length() == 0 || machineUUID.length() == 0 || machineAPIKey.length() == 0) {
    Serial.println("⚠ Machine not fully configured - skipping transmission");
...
void initializeWiFiManager() {
  ...
  if(savedSSID.length() == 0) {
    ...
    if(!wifiManager.autoConnect("ESP32_HVAC_Setup")) {
...
void saveConfigCallback() {
  ...
  // Note: Actual parameter retrieval and saving happens in initializeWiFiManager()
}
