/*
 * ============================================
 * ESP32 R32 - Production HVAC Monitoring System
 * With WiFiManager, Supabase Integration, and Smart Scheduling
 * ============================================
 * 
 * VERSION: 2.0.1
...
const unsigned long HTTP_POST_TIMEOUT = 60000;        // 60 seconds
...
void setup() {
  ...
  Serial.println("\n\n========================================");
  Serial.println("  ESP32 R32 - Production HVAC Monitor");
  ...
  // Add custom parameters for Supabase
  WiFiManagerParameter customSupabaseUrl("supabase_url", "Supabase URL", "", 100);
  WiFiManagerParameter customSupabaseKey("supabase_key", "Supabase Anon Key", "", 200);
  WiFiManagerParameter customMachineUUID("machine_uuid", "Machine UUID", "", 40);
  
  wifiManager.addParameter(&customSupabaseUrl);
  wifiManager.addParameter(&customSupabaseKey);
  ...
  // Try to load saved Supabase config
  supabaseUrl = preferences.getString("supabase_url", "");
  supabaseAnonKey = preferences.getString("supabase_key", "");
  machineUUID = preferences.getString("machine_uuid", "");
  
  if(supabaseUrl.length() > 0) {
    Serial.println("✓ Supabase config loaded from memory");
    Serial.println("  URL: " + supabaseUrl);
    Serial.println("  Machine UUID: " + machineUUID);
  } else {
    Serial.println("⚠ No Supabase config found - will prompt on first WiFi setup");
  }
...
void stateDataSend(unsigned long currentMillis) {
  ...
  if(supabaseUrl.length() == 0 || machineUUID.length() == 0) {
    Serial.println("⚠ Supabase not configured - launching config portal...");
    launchConfigPortal();
    currentState = STATE_WIFI_DISCONNECT;
    return;
  }
...
bool sendToSupabase(float tMotor, float tExterior, float tInterior,
  ...
  doc["derived_state"] = (bStatus == "ON") ? "on" : "off"; // Pump status
  ...
  p1["input"] = 1;
  p1["label"] = "exhaust";
  p1["value"] = (yStatus == "ON") ? 1 : 0;
  p1["voltage"] = yVolt;
  
  JsonObject p2 = inputs.createNestedObject();
  p2["input"] = 2;
  p2["label"] = "fan";
  p2["value"] = (gStatus == "ON") ? 1 : 0;
  p2["voltage"] = gVolt;
  p2["speed"] = fanSpeed;
  
  JsonObject p3 = inputs.createNestedObject();
  p3["input"] = 3;
  p3["label"] = "pump";
  p3["value"] = (bStatus == "ON") ? 1 : 0;
  p3["voltage"] = bVolt;
  
  JsonObject p4 = inputs.createNestedObject();
  p4["input"] = 4;
  p4["label"] = "drain";
  p4["value"] = (blStatus == "ON") ? 1 : 0;
  p4["voltage"] = blVolt;
...
void saveConfigCallback() {
  ...
  supabaseUrl = wifiManager.server->arg("supabase_url");
  supabaseAnonKey = wifiManager.server->arg("supabase_key");
  machineUUID = wifiManager.server->arg("machine_uuid");
  
  // Save to preferences
  preferences.putString("supabase_url", supabaseUrl);
  preferences.putString("supabase_key", supabaseAnonKey);
...
// ============================================
// Check WiFi Safety Net (Called from loop)
// ============================================
// This is called every loop iteration to ensure WiFi doesn't stay on too long
// Already integrated into main loop via checkWiFiTimeout()
