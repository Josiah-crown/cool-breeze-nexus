-- Migration 26c: Fix function to match DIRECT_FIX_CONNECTION.sql logic exactly
-- The direct fix works, so let's make the function use the same logic
...
BEGIN
  ...
  -- Use EXACT same logic as DIRECT_FIX_CONNECTION.sql
  IF v_last_cirrus IS NOT NULL AND v_last_coolbreeze IS NOT NULL THEN
    ...
    IF v_last_cirrus.timestamp > v_last_coolbreeze.timestamp THEN
      -- Use Cirrus data - match DIRECT_FIX logic exactly
      v_minutes_ago := EXTRACT(EPOCH FROM (NOW() - v_last_cirrus.timestamp))/60;
      -- Use same logic as direct fix: c.is_connected = true AND minutes <= 15
  ...
  ELSIF v_last_cirrus IS NOT NULL THEN
    -- Only Cirrus data exists - use EXACT same logic as DIRECT_FIX
    v_minutes_ago := EXTRACT(EPOCH FROM (NOW() - v_last_cirrus.timestamp))/60;
    -- Match direct fix: c.is_connected = true AND minutes <= 15
    v_is_connected := (v_last_cirrus.is_connected = true) AND (v_minutes_ago <= 15);
...
COMMENT ON FUNCTION public.update_machine_from_latest_reading IS 'Updates machines table from latest reading. Uses same logic as DIRECT_FIX_CONNECTION.sql (is_connected = true instead of IS TRUE).';
