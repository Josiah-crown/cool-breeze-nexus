-- Fix connection check logic
-- The issue: Function should use is_connected from processing table AND verify timestamp is recent
-- This ensures we trust the processing table's calculation but also enforce a 15-minute window
...
BEGIN
  ...
  IF v_last_cirrus IS NOT NULL AND v_last_coolbreeze IS NOT NULL THEN
    ...
    IF v_last_cirrus.timestamp > v_last_coolbreeze.timestamp THEN
      ...
      -- Use the is_connected value from the cirrus table AND verify it's within 15 minutes
      -- The cirrus table uses 10 minutes, but machines table uses 15 minutes
      v_is_connected := v_last_cirrus.is_connected AND (v_minutes_ago <= 15);
    ...
    ELSE
      ...
      v_minutes_ago := EXTRACT(EPOCH FROM (NOW() - v_last_reading))/60;
      v_is_connected := v_last_coolbreeze.is_connected AND (v_minutes_ago <= 15);
  ...
  ELSIF v_last_cirrus IS NOT NULL THEN
    ...
    v_last_reading := v_last_cirrus.timestamp;
    v_minutes_ago := EXTRACT(EPOCH FROM (NOW() - v_last_reading))/60;
    -- Use the is_connected value from cirrus table AND verify it's within 15 minutes
    v_is_connected := v_last_cirrus.is_connected AND (v_minutes_ago <= 15);
...
COMMENT ON FUNCTION public.update_machine_from_latest_reading IS 'Updates machines table from latest reading. Uses is_connected from processing table AND verifies it is within 15 minutes.';
