-- Migration 26e: Use a helper function that matches DIRECT_FIX exactly
-- This creates a helper function that does the exact same logic as DIRECT_FIX_CONNECTION.sql

-- Helper function that calculates connection status (matches DIRECT_FIX logic exactly)
CREATE OR REPLACE FUNCTION public.calculate_connection_status(
  p_is_connected BOOLEAN,
  p_timestamp TIMESTAMPTZ
)
RETURNS BOOLEAN AS $$
BEGIN
  -- Exact same logic as DIRECT_FIX_CONNECTION.sql
  RETURN (p_is_connected = true) AND (EXTRACT(EPOCH FROM (NOW() - p_timestamp))/60 <= 15);
END;
...
BEGIN
  ...
  IF v_last_cirrus IS NOT NULL AND v_last_coolbreeze IS NOT NULL THEN
    ...
    IF v_last_cirrus.timestamp > v_last_coolbreeze.timestamp THEN
      v_is_connected := public.calculate_connection_status(v_last_cirrus.is_connected, v_last_cirrus.timestamp);
    ...
    ELSE
      ...
      v_is_connected := public.calculate_connection_status(v_last_coolbreeze.is_connected, v_last_coolbreeze.timestamp);
...
COMMENT ON FUNCTION public.calculate_connection_status IS 'Helper function that calculates connection status using exact same logic as DIRECT_FIX_CONNECTION.sql';
COMMENT ON FUNCTION public.update_machine_from_latest_reading IS 'Updates machines table from latest reading. Uses calculate_connection_status helper function.';
