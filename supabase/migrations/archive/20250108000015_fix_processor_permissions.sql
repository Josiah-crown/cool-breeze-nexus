-- Fix permissions for processor functions
-- These functions need to read from machines table, but RLS was blocking them
-- Making them SECURITY DEFINER allows them to bypass RLS

-- Fix Cirrus processor
...
COMMENT ON FUNCTION public.process_cirrus_reading IS 'Automatically processes readings_raw entries into CIRRUS table for evaporative cooler machines. SECURITY DEFINER allows bypassing RLS to read from machines table.';

-- Fix CoolBreeze processor function
...
COMMENT ON FUNCTION public.process_coolbreeze_reading IS 'Automatically processes readings_raw entries into CoolBreeze table for HVAC machines. SECURITY DEFINER allows bypassing RLS to read from machines table.';
