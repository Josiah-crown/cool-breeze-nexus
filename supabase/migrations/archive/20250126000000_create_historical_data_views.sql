-- Create Historical Data Views/Functions for Time Periods
-- This provides optimized queries for different time ranges with appropriate aggregation

-- ============================================================================
-- FUNCTION: Get Historical Data for a Machine by Period
-- ============================================================================
-- This function aggregates data appropriately for each time period:
-- - 24h: All readings (no aggregation needed)
  ...
  IF v_interval = INTERVAL '0 seconds' THEN
    ...
    RETURN QUERY EXECUTE format('
      SELECT 
        ...
        r.voltage,
        CASE 
          WHEN %L = ''alliance'' THEN NULL::INTEGER
    ...
    ', p_table_name, p_table_name) USING p_machine_id, v_start_time;
  ELSE
    ...
    RETURN QUERY EXECUTE format('
      SELECT 
        ...
        BOOL_OR(r.is_cooling) AS is_cooling,
    ...
    ', p_table_name, p_table_name) USING p_machine_id, v_start_time, v_interval;
  END IF;
END;
$$;
...
- 7d: 10-minute averages
- 30d: 1-hour averages (FIXED: now shows hourly data, not daily)
- 1y: 1-day averages (365 data points)
Returns: timestamp, motor_temp, current, ambient_temp, duct_temp, delta_t, fan_active, is_cooling, has_water, pump_active, power, voltage, fan_speed (NULL for heatpumps/alliance)
Note: fan_speed is NULL for alliance table since heatpumps do not have fans
Parameters: machine_id, period (24h|7d|30d|1y), table_name (cirrus|coolbreeze|alliance)';

-- ============================================================================
-- VIEW: Historical Data Summary (for debugging/verification)
-- ============================================================================
