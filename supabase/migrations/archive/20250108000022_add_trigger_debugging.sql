-- Add debugging and fix potential issues with connection status update
-- This version adds better error handling and ensures triggers fire correctly
...
-- Test function to check connection status for a specific machine
CREATE OR REPLACE FUNCTION public.test_machine_connection_status(p_machine_id UUID)
RETURNS TABLE(
  machine_id UUID,
  machine_name TEXT,
  machine_type TEXT,
  manufacturer TEXT,
  is_connected_in_machines BOOLEAN,
  last_reading_time TIMESTAMPTZ,
  minutes_ago NUMERIC,
  should_be_connected BOOLEAN,
  has_cirrus_data BOOLEAN,
  has_coolbreeze_data BOOLEAN,
  latest_cirrus_timestamp TIMESTAMPTZ,
  latest_coolbreeze_timestamp TIMESTAMPTZ
) AS $$
BEGIN
  RETURN QUERY
  SELECT 
    ...
    (GREATEST(
      COALESCE(MAX(c.timestamp), '1970-01-01'::timestamptz),
      COALESCE(MAX(cb.timestamp), '1970-01-01'::timestamptz)
    ) >= (NOW() - INTERVAL '15 minutes')) as should_be_connected,
    (MAX(c.timestamp) IS NOT NULL) as has_cirrus_data,
    (MAX(cb.timestamp) IS NOT NULL) as has_coolbreeze_data,
    MAX(c.timestamp) as latest_cirrus_timestamp,
    MAX(cb.timestamp) as latest_coolbreeze_timestamp
  FROM public.machines m
  LEFT JOIN public.cirrus c ON c.machine_id = m.id
  LEFT JOIN public.coolbreeze cb ON cb.machine_id = m.id
  WHERE m.id = p_machine_id
  GROUP BY m.id, m.name, m.type, m.manufacturer, m.is_connected;
END;
$$ LANGUAGE plpgsql;

COMMENT ON FUNCTION public.test_machine_connection_status IS 'Test function to diagnose connection status issues. Shows what the connection status should be vs what it is.';
