-- Let site viewers resolve owner / company / member display names (profiles RLS is own-row only for clients).
...
AS $$
  ...
  FROM (
    ...
    UNION
    SELECT s.company_id AS user_id
    FROM public.sites s
    WHERE s.id = p_site_id AND s.company_id IS NOT NULL
    UNION
    SELECT sm.user_id
...
COMMENT ON FUNCTION public.get_site_display_names(UUID) IS
  'Display names for site owner, company, and members — callable by anyone who can access the site.';
