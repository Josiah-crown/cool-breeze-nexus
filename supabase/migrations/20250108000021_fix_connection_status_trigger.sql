-- Fix connection status trigger to handle all machine types correctly
-- The issue is that the trigger might not be matching machine type/manufacturer correctly

-- Drop and recreate the update function with better logic
...
BEGIN
  ...
  -- Check both tables for latest reading (some machines might have data in both during migration)
...
COMMENT ON FUNCTION public.update_machine_from_latest_reading IS 'Updates machines table from latest reading in either cirrus or coolbreeze table. Checks both tables and uses most recent data. Sets readings to 0 if disconnected (>15 min).';
